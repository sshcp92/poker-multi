...
# 2. 데이터 엔진
# ==========================================
DATA_FILE = "poker_v15_final.json"
...
if curr_idx != my_seat and data['phase'] != 'GAME_OVER':
    if not curr_p['is_human']:
        ...
        if act == "Fold": curr_p['status'] = 'folded'; act_str = "폴드"
        elif act == "Check": act_str = "체크"
        elif act == "Call": act_str = f"콜 ({actual})"
        elif act == "Raise": act_str = f"레이즈 ({curr_p['bet']})"
...
with col2:
    ...
    if curr_idx == my_seat and data['phase'] != 'GAME_OVER':
        ...
        btn = "체크" if to_call == 0 else f"콜 ({to_call})"
        ...
        if me['stack'] > min_r:
            ...
            if st.button("레이즈 확정", use_container_width=True):
                ...
                data['last_log'] = f"📢 {me['name']}: 레이즈 ({val})"
                for p in data['players']:
