...
def init_game_data():
    ...
    players[1]['stack'] -= 100
    players[1]['bet'] = 100
    players[1]['action'] = 'SB 100'
    players[1]['has_acted'] = True
    players[2]['stack'] -= 200
    players[2]['bet'] = 200
    players[2]['action'] = 'BB 200'
...
def make_comm_card(card):
    ...
    rank = r_str(card[0])
    suit = card[1]
    return f"<span class='card-span comm-card-span' style='color:{color}'>{rank}{suit}</span>"
...
def get_hand_strength_detail(hand):
    ...
    for i in range(len(unique) - 4):
        if unique[i] - unique[i+4] == 4:
            ...
            high_s = unique[i]
            break
    if not is_str and set([12, 3, 2, 1, 0]).issubset(set(ranks)):
        is_str = True
        high_s = 3
    ...
    if is_flush and is_str:
        return (8, ranks, "스트레이트 플러시")
    if sorted_groups[0][0] == 4:
        return (7, ranks, f"포카드 ({r_str(RANKS[sorted_groups[0][1]])})")
    if sorted_groups[0][0] == 3 and sorted_groups[1][0] >= 2:
        return (6, ranks, f"풀하우스 ({r_str(RANKS[sorted_groups[0][1]])})")
    if is_flush:
        return (5, ranks, "플러시")
    if is_str:
        return (4, ranks, "스트레이트")
    if sorted_groups[0][0] == 3:
        return (3, ranks, f"트리플 ({r_str(RANKS[sorted_groups[0][1]])})")
    if sorted_groups[0][0] == 2 and sorted_groups[1][0] == 2:
        return (2, ranks, f"투페어 ({r_str(RANKS[sorted_groups[0][1]])} & {r_str(RANKS[sorted_groups[1][1]])})")
    if sorted_groups[0][0] == 2:
        return (1, ranks, f"원페어 ({r_str(RANKS[sorted_groups[0][1]])})")
...
# ==========================================
# 5. 입장 처리 (유저 입장 시 봇 제거 및 새 유저 생성)
# ==========================================
if 'my_seat' not in st.session_state:
    ...
    if col1.button("입장하기", type="primary"):
        ...
        # 신규 입장 시
        if target == -1:
            if not data['players'][4]['is_human']:
                target = 4
            else:
                for i in range(9): 
                    if not data['players'][i]['is_human']:
                        target = i
                        break
            
            if target != -1:
                # [수정] 기존 봇 데이터를 싹 날리고 새 유저 데이터를 생성하여 앉힘
                data['players'][target] = {
                    'name': u_name,
                    'seat': target + 1,
                    'stack': 100000, # 항상 100,000칩을 가지고 시작
                    'hand': [data['deck'].pop(), data['deck'].pop()], # 새로운 카드 지급
                    'bet': 0,
                    'status': 'alive',
                    'action': '입장완료',
                    'is_human': True,
                    'role': data['players'][target]['role'], # 딜러/블라인드 역할은 유지
                    'has_acted': False,
                    'style': 'Hero'
                }
                save_data(data)
...
with col_controls:
    ...
    if data['phase'] == 'GAME_OVER':
        ...
        if not curr_p['is_human']:
            ...
            if act == "Call" and actual == 0:
                act_str = "체크"
            elif act == "Fold":
                act_str = "폴드"
                curr_p['status'] = 'folded'
            else:
                act_str = f"{act} ({curr_p['bet']})"
